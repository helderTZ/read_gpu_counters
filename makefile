CC = gcc
CFLAGS = `pkg-config --cflags libdrm cairo`  
LDFLAGS = `pkg-config --libs libdrm cairo` 

CFLAGS += -g -O0	# optimization and debugging flags
CFLAGS += -fdump-rtl-expand	# necessary to create callgraph

# OpenCL linker flags
LDFLAGS += -I/opt/intel/opencl-sdk/include /opt/intel/opencl-sdk/lib64/libOpenCL.so

SOURCES = read_gpu_counters.c
