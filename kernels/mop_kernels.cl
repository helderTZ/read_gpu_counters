...
/******************************************************
  * Scalar single-precision load+store no loop
  * ***************************************************/
 
__kernel void __attribute__((vec_type_hint(float))) scalar_sp_load_store_kernel(__global float *src, __global float *dst) {
	int i = get_global_id(0);
	dst[i] = src[i];
}

/******************************************************
  * Vector single-precision load+store no loop
  * ***************************************************/
 
__kernel void __attribute__((vec_type_hint(float2))) vect2_sp_load_store_kernel(__global float2 *src, __global float2 *dst) {
	int i = get_global_id(0);
	dst[i] = src[i];
}

__kernel void __attribute__((vec_type_hint(float4))) vect4_sp_load_store_kernel(__global float4 *src, __global float4 *dst) {
	int i = get_global_id(0);
	dst[i] = src[i];
}

__kernel void __attribute__((vec_type_hint(float8))) vect8_sp_load_store_kernel(__global float8 *src, __global float8 *dst) {
	int i = get_global_id(0);
	dst[i] = src[i];
}

__kernel void __attribute__((vec_type_hint(float16))) vect16_sp_load_store_kernel(__global float16 *src, __global float16 *dst) {
	int i = get_global_id(0);
	dst[i] = src[i];
}
...
/******************************************************
  * Scalar double-precision load+store no loop
  * ***************************************************/
 
__kernel void __attribute__((vec_type_hint(double))) scalar_dp_load_store_kernel(__global double *src, __global double *dst) {
	int i = get_global_id(0);
	dst[i] = src[i];
}

/******************************************************
  * Vector double-precision load+store no loop
  * ***************************************************/
 
__kernel void __attribute__((vec_type_hint(double2))) vect2_dp_load_store_kernel(__global double2 *src, __global double2 *dst) {
	int i = get_global_id(0);
	dst[i] = src[i];
}

__kernel void __attribute__((vec_type_hint(double4))) vect4_dp_load_store_kernel(__global double4 *src, __global double4 *dst) {
	int i = get_global_id(0);
	dst[i] = src[i];
}

__kernel void __attribute__((vec_type_hint(double8))) vect8_dp_load_store_kernel(__global double8 *src, __global double8 *dst) {
	int i = get_global_id(0);
	dst[i] = src[i];
}

__kernel void __attribute__((vec_type_hint(double16))) vect16_dp_load_store_kernel(__global double16 *src, __global double16 *dst) {
	int i = get_global_id(0);
	dst[i] = src[i];
}
...
__kernel void __attribute__((vec_type_hint(float))) scalar_sp_load_store_iter_1024_volatile_kernel(__global float *src, __global float *dst) {
	int i = get_global_id(0);
	volatile int j;
	for(j = 0; j < 1024; j++)
		dst[i]= src[i];
...
__kernel void __attribute__((vec_type_hint(float2))) vect2_sp_load_store_iter_1024_volatile_kernel(__global float2 *src, __global float2 *dst) {
	int i = get_global_id(0);
	volatile int j;
	for(j = 0; j < 1024; j++)
		dst[i]= src[i];
...
__kernel void __attribute__((vec_type_hint(float4))) vect4_sp_load_store_iter_1024_volatile_kernel(__global float4 *src, __global float4 *dst) {
	int i = get_global_id(0);
	volatile int j;
	for(j = 0; j < 1024; j++)
		dst[i]= src[i];
...
__kernel void __attribute__((vec_type_hint(float8))) vect8_sp_load_store_iter_1024_volatile_kernel(__global float8 *src, __global float8 *dst) {
	int i = get_global_id(0);
	volatile int j;
	for(j = 0; j < 1024; j++)
		dst[i]= src[i];
...
__kernel void __attribute__((vec_type_hint(float16))) vect16_sp_load_store_iter_1024_volatile_kernel(__global float16 *src, __global float16 *dst) {
	int i = get_global_id(0);
	volatile int j;
	for(j = 0; j < 1024; j++)
		dst[i]= src[i];
 ...
 __kernel void __attribute__((vec_type_hint(float))) scalar_dp_load_store_iter_1024_volatile_kernel(__global double *src, __global double *dst) {
	int i = get_global_id(0);
	volatile int j;
	for(j = 0; j < 1024; j++)
		dst[i] = src[i];
 ...
 __kernel void __attribute__((vec_type_hint(float2))) vect2_dp_load_store_iter_1024_volatile_kernel(__global double2 *src, __global double2 *dst) {
	int i = get_global_id(0);
	volatile int j;
	for(j = 0; j < 1024; j++)
		dst[i] = src[i];
 ...
 __kernel void __attribute__((vec_type_hint(float4))) vect4_dp_load_store_iter_1024_volatile_kernel(__global double4 *src, __global double4 *dst) {
	int i = get_global_id(0);
	volatile int j;
	for(j = 0; j < 1024; j++)
		dst[i] = src[i];
 ...
 __kernel void __attribute__((vec_type_hint(float8))) vect8_dp_load_store_iter_1024_volatile_kernel(__global double8 *src, __global double8 *dst) {
	int i = get_global_id(0);
	volatile int j;
	for(j = 0; j < 1024; j++)
		dst[i] = src[i];
 ...
 __kernel void __attribute__((vec_type_hint(float16))) vect16_dp_load_store_iter_1024_volatile_kernel(__global double16 *src, __global double16 *dst) {
	int i = get_global_id(0);
	volatile int j;
	for(j = 0; j < 1024; j++)
		dst[i] = src[i];
